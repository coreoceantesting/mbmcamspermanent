<div>
            ...
            <table class="table table-hover" id="list_table">
                <thead>
                    <tr>
                        ...
                        <th style="min-width: 120px">{{ $pageType == 'half_day' ? 'Date' : 'From Date' }}</th>
                ...
                <tbody>
                    ...
                    @foreach ($leaveRequests as $request)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-primary" target="_blank" href="{{asset($request->document->path)}}">File</a>
                            </td>
                            <td>
                                <button class="btn btn-danger remove-element px-2 py-1" title="Delete Leave" data-id="{{ $request->id }}"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <div class="row">
        <div class="d-flex justify-content-between">
            <div>
                Total Employee: {{ $leaveRequests->total() }}
            </div>
            <div>
                {{ $leaveRequests->links() }}
            </div>
